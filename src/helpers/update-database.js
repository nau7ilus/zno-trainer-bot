...
async function main() {
  ...
  testIDs.forEach(async (testID) => {
    ...
    for (let i = 1; i < 24; i++) {
      ...
      if (taskType === "simple") answers = parsedTask.indexOf("true");
      else if (taskType === "table") {
        const tableAnswers = [];
        parsedTask.forEach((el, i) => tableAnswers.push([i + 1, el.indexOf("true")]));
        answers = tableAnswers;
      }
      try {
        await Task.create({ type: taskType, photoURL, answers, ids: { test: testID, task: i } });
      } catch (err) {
        console.error(err);
      }
    }
