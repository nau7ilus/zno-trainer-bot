...
module.exports = async ({ telegram, i18n }, userID, hideIncognito = true) => {
  // Ищем пользователей и фильтруем сразу в запросе
  const users = await User.find().sort('-points').limit(10).cache(10);
  ...
  for await (const [i, user] of users.entries()) {
    ...
    const userName = firstName && (!incognito || !hideIncognito) ? firstName : i18n.t('rating.incognito');
    const formattedPoints = formatInt(points);
    const rightAnswersPercent = ((answers / askedQuestions) * 100).toFixed(1);
    const userRatingData = `<b>${i + 1}. ${userName} — ${isNaN(formattedPoints) ? 0 : formattedPoints} | ${
      isNaN(rightAnswersPercent) ? 0 : rightAnswersPercent
    }%</b>`;
