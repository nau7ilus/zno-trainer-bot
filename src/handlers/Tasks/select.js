...
module.exports = class extends Handler {
  ...
  run(ctx) {
    if (ctx.match[1] === 'subject') {
      send(ctx, ctx.i18n.t('select.subjects.message'), [...selectSubject(ctx), backButton(ctx, 'start')]);
    }

    if (ctx.match[1] === 'types') {
      send(ctx, ctx.i18n.t('select.types.message'), [
        ...selectTypes(ctx, ctx.match[2]),
        backButton(ctx, 'select::subject'),
      ]);
    }
    ...
    if (ctx.match[1] === 'themes') {
      ...
      if (!ctx.match[3]) {
        send(ctx, ctx.i18n.t('select.themes.message', { themes: Object.keys(themes) }), [
          ...chunk(
        ...
        send(ctx, ctx.i18n.t('select.themes.message', { themes: Object.values(themes)[ctx.match[3]] }), markup);
      }
    }
  }
};
